...
// NOTE: The Graph's hosted service (api.thegraph.com/subgraphs/name/...) was
// permanently shut down on 12 June 2024 — every request to it now returns a
// "removed" error, which used to make this function ALWAYS fall back to the
// hardcoded FALLBACK_POOLS below. DefiLlama's public Yields API is free, needs
// no key, has no rate limit for normal traffic, and still tracks live
// Uniswap V3 pool data (project: "uniswap-v3"), so we use that instead.
...
export async function fetchPools(): Promise<PoolData[]> {
  try {
    const r = await fetch(DEFILLAMA_POOLS);
    if (!r.ok) return FALLBACK_POOLS;
    const { data } = await r.json() as { data?: DefiLlamaPool[] };
    if (!data?.length) return FALLBACK_POOLS;

    const uniPools = data
      .filter((p) => p.project === 'uniswap-v3' && p.chain === 'Ethereum' && p.tvlUsd > 1_000_000)
      .sort((a, b) => b.tvlUsd - a.tvlUsd)
      .slice(0, 20);

    if (!uniPools.length) return FALLBACK_POOLS;

    return uniPools.map((p): PoolData => {
      const [sym0, sym1] = p.symbol.split('-');
      const feeTier = p.poolMeta ? Math.round(parseFloat(p.poolMeta) * 10000) : 3000;
      const vol24 = p.volumeUsd1d ?? 0;
      const fees24 = vol24 * (feeTier / 1_000_000);
      return {
        id: p.pool,
        token0: p.underlyingTokens?.[0] ?? '',
        token1: p.underlyingTokens?.[1] ?? '',
        token0Symbol: sym0 ?? p.symbol,
        token1Symbol: sym1 ?? '',
        feeTier,
        tvlUsd: p.tvlUsd,
        volume24h: vol24,
