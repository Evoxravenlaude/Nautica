...
if [ -n "$ALCHEMY_KEY" ]; then
  echo "$ALCHEMY_KEY" | npx wrangler secret put ALCHEMY_KEY
  echo "✅  ALCHEMY_KEY"
else
...
if [ -n "$ZERO_X_KEY" ]; then
  echo "$ZERO_X_KEY" | npx wrangler secret put ZERO_X_KEY
  echo "✅  ZERO_X_KEY"
else
...
if [ -n "$RESERVOIR_KEY" ]; then
  echo "$RESERVOIR_KEY" | npx wrangler secret put RESERVOIR_KEY
  echo "✅  RESERVOIR_KEY"
else
