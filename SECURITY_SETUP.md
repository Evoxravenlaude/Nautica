...
```bash
npx wrangler secret put ALCHEMY_KEY    # from dashboard.alchemy.com
npx wrangler secret put ZERO_X_KEY     # from dashboard.0x.org
```

> ⚠️ These names must match exactly what `worker.js` reads (`env.ALCHEMY_KEY`,
> `env.ZERO_X_KEY`). If you previously ran `wrangler secret put ALCHEMY_API_KEY`
> or `ZERO_X_API_KEY`, the worker will silently ignore them — run
> `npx wrangler secret list` to check what's currently set, and `wrangler secret delete`
> any wrongly-named ones.
